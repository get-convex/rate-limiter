...
describe.each(["token bucket", "fixed window"] as const)(
  ...
  (kind) => {
    ...
    test("rejects a rate limit with a negative shard count", async () => {
      const t = convexTest(schema, modules);
      await expect(
        t.mutation(api.lib.creditRateLimit, {
          name: "backwards",
          config: { kind, rate: 3, period: Hour, shards: -3 },
          count: 1,
        }),
      ).rejects.toThrow("Shards must be a positive number");
    });
...
describe("asynchronous configs", () => {
  ...
  test("can't be credited by creditRateLimit", async () => {
    const t = convexTest(schema, modules);
    await expect(
      t.mutation(api.lib.creditRateLimit, { name: "async", config, count: 1 }),
    ).rejects.toThrow(
      'Rate limit config for async has `applyUpdates: "asynchronously"`',
    );
  });

  test("a negative credit can't be queued either", async () => {
    const t = convexTest(schema, modules);
    await expect(
      t.mutation(api.lib.enqueueUpdates, {
        updates: [{ kind: "credit", name: "async", count: -1, config }],
      }),
    ).rejects.toThrow("Rate limit async credit count -1 is negative");
  });
