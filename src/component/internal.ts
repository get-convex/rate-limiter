...
/**
 * Pick two distinct shards at random, so a request only takes a read
 * dependency on two shards instead of all of them. The second shard is only
 * meaningful when there are at least MIN_CHOOSE_TWO shards.
 */
...
export async function creditRateLimitSharded(
  ...
  const [one, two] = getTwoRandomShards(shards);
